...
class kpi_setting_dsm_activity_lines_models(models.Model):
    ...
    # def _depends_project_id(self):
    #     for rec in self:
            # rec_lines = self.env['kpi_setting_dsm_pm_lines'].search([
            #     ('project_name', '=', rec.project_id),
            # ])
            # rec.kpi_name = rec_lines.id
